<template>
  <div id="app">
    <div style="padding: 0px 50px; width: 80%;">
      <HighLight
        :value="data"
        :template="template"
        :modes="toolBarList"
      ...
      >
        <ToolBar v-if="visible" :visible="visible" @onCancel="onToolBarCancel">
          <template v-if="action == 'add'">
            <div class="note-tool-item" @click="handleSubmitLineation">
              <span class="iconfont icon-huaxian"></span>
              <i>划线</i>
            </div>
            <div class="note-tool-item" @click="openNote">
              <span class="iconfont icon-edit"></span>
              <i>笔记</i>
            </div>
            <div class="note-tool-item" @click="copy">
              <span class="iconfont icon-fuzhi"></span>
              <i>复制</i>
            </div>
          </template>
          <template v-if="action === 'update' && updateSelectedMode != 2">
            <div class="note-tool-item" @click="onToolBarHuaxianCancel">
              <span class="iconfont icon-huaxian"></span>
              <i>取消划线</i>
            </div>
            <div
              v-if="updateSelectedMode === 3"
              class="note-tool-item"
              @click="onToolBarEditShow"
            >
              <span class="iconfont icon-edit"></span>
              <i>查看笔记</i>
            </div>
        ...
        >
          <div class="edit-textarea-wrap">
            <textarea class="edit-textarea" v-model="textAreaValue"></textarea>
            <button @click="handleSubmitNote">提交</button>
            <button v-if="selectedUpdateNoteID" @click="handleDeleteNote">
              删除
            </button>
          </div>
        </ToolBar>
...
export default {
  ...
  data() {
    return {
      ...
      template: `<p>你好，我是winter，今天我们继续来看浏览器的相关内容。</p><p>我在上一篇文章中，简要介绍了浏览器的工作大致可以分为6个阶段，我们昨天讲完了第一个阶段，也就是通讯的部分：浏览器使用HTTP协议或者HTTPS协议，向服务端请求页面的过程。</p><p>今天我们主要来看两个过程：如何解析请求回来的HTML代码，DOM树又是如何构建的。<br>
...
<li><a href="http://www.w3.org/html/wg/drafts/html/master/syntax.html#tree-construction">http://www.w3.org/html/wg/drafts/html/master/syntax.html#tree-construction</a></li>
  ...
  mounted() {
    this.data = (getLocalStorage(tempName) || []).filter(
      (item) => item.mode !== 'temp',
    )
  ...
  methods: {
    copy() {
      copyToShearPlate(this.selectText.text)
      this.onToolBarCancel()
    },
    openNote() {
      this.visible = false
      this.textAreaVisible = true
      this.rerender = getUUID()
      setSelectRange(this.selectText)
      this.$set(this.data, this.data.length, {
        ...this.selectText,
        mode: 'temp',
        id: getUUID(),
      })
      setLocalStorage(tempName, this.data)
    },
    onAdd(selectText) {
      debugger
      this.action = 'add'
    ...
    onUpdate(list = []) {
      console.log(list)
      const hasHuaxian = !isEmpty(
        list.filter((item) => item.mode === 'huaxian'),
      )
      ...
      this.textAreaVisible = hasEdit ? true : false
      this.textAreaValue = param.textAreaValue
      this.selectedUpdateNoteID = hasEdit ? edit.id : ''
      this.selectTextMap = param.selectTextMap
      this.updateSelectedMode = param.updateSelectedMode
      this.action = param.action
      this.selectText = param.selectText
      // if (hasEdit) {
      //   this.rerender = getUUID();
      // }
    },
    ...
    onToolBarTextAreaCancel() {
      let arr = this.data.filter((item) => item.mode !== 'temp')
      setLocalStorage(tempName, arr)
      this.selectedUpdateNoteID = ''
      this.textAreaValue = ''
      this.textAreaVisible = false
    },

    onToolBarHuaxianCancel() {
      const { huaxian } = this.selectTextMap
      if (Object.keys(huaxian).length > 0) {
        let arr = this.data.filter((item) => item != huaxian)
        this.data = arr
        setLocalStorage(tempName, arr)
        this.visible = false
        this.update = getUUID()
    ...
    onToolBarEditShow() {
      const { edit } = this.selectTextMap || {}
      if (!edit) return
      setSelectRange(edit)
      this.visible = false
      this.textAreaValue = edit.note
      this.textAreaVisible = true
      this.selectedUpdateNoteID = edit.id
    },

    handleSubmitNote() {
      if (this.action === 'add') {
        const index = this.data.findIndex((d) => d.mode === 'temp')
        const insertData = {
          ...this.data[index],
          mode: 'edit',
        ...
        this.data.splice(index === -1 ? 0 : index, 1, insertData)
      } else if (this.action === 'update') {
        const { edit } = this.selectTextMap
        if (!edit) return
        edit.note = this.textAreaValue
        console.log('更新笔记成功')
      }
      setLocalStorage(tempName, this.data)
      this.selectedUpdateNoteID = ''
    ...
    handleDeleteNote() {
      let curArr = this.data.filter(
        (item) => item.id !== this.selectedUpdateNoteID,
      )
      this.data = curArr
      setLocalStorage(tempName, curArr)
      this.selectedUpdateNoteID = ''
      this.textAreaValue = ''
      this.textAreaVisible = false
      this.update = getUUID()
    ...
    handleSubmitLineation() {
      this.onToolPaneAdd('huaxian')
    },
  },
}
</script>
